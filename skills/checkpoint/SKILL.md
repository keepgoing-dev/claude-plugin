...
allowed-tools: ["mcp__plugin_keepgoing_keepgoing__save_checkpoint"]
---
...
Call `mcp__plugin_keepgoing_keepgoing__save_checkpoint` with:
- `summary`: The message from `$ARGUMENTS`, or your inferred 1-2 sentence summary of what was accomplished and why. No file paths or implementation details - just what changed and the reason.
- `nextStep`: What should be done next (infer from context if not obvious).
- `blocker`: Any blocker if one is apparent, otherwise omit.
